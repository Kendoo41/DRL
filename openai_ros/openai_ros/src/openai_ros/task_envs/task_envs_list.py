...
def RegisterOpenAI_Ros_Env(task_env, max_episode_steps=1000):
    """
    ...
    elif task_env == "ABB_random_world":

        register(
            id=task_env,
            entry_point="openai_ros.task_envs.abb.abb_random_world:AbbRandomWorldEnv",
            max_episode_steps=max_episode_steps,
        )
        from openai_ros.task_envs.abb import abb_random_world

    elif task_env == "NachiReach-v0":

        register(
            id=task_env,
            entry_point="openai_ros.task_envs.nachi.nachi_random_world:NachiRandomWorldEnv",
            max_episode_steps=max_episode_steps,
        )
